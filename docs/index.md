---
layout: default
title: Infrared Image Synthesis
---

# Infrared Image Synthesis

---

## Table of Contents

- [1. Introduction](#1-introduction)
  - [1.1. Problem Definitions](#11-problem-definitions)
- [2. Fundamentals of Infrared Images](#2-fundamentals-of-infrared-images)
  - [2.1. Imaging System](#21-imaging-system)
    - [Difficulties with the Lens](#difficulties-with-the-lens)
    - [Difficulties with Sensors](#difficulties-with-sensors)
    - [Difficulties with Processors](#difficulties-with-processors)
  - [2.2. IR Image Synthesis Model](#22-ir-image-synthesis-model)
  - [2.3. Applications](#23-applications)
    - [Medical biochemical engineering](#medical-biochemical-engineering)
      - [Pharmaceutical industry](#pharmaceutical-industry)
      - [Medical science](#medical-science)
      - [Cellular observations](#cellular-observations)
      - [Fluorescence microscopy](#fluorescence-microscopy)
    - [Vision tasks](#vision-tasks)
      - [Image conversion](#image-conversion)
      - [Multispectral matching](#multispectral-matching)
      - [Targets detection](#targets-detection)
      - [Face recognition](#face-recognition)
    - [Other engineering tasks](#other-engineering-tasks)
      - [Automated vehicle](#automated-vehicle)
      - [Remote sensing](#remote-sensing)
      - [Terrain models](#terrain-models)
      - [Land surface](#land-surface)
      - [Food quality control](#food-quality-control)
      - [Agricultural management](#agricultural-management)
      - [Water resource management](#water-resource-management)
      - [Star formation](#star-formation)

---

## 1. Introduction

Write your introduction here.

### 1.1. Problem Definitions

Define the problem, task formulation, inputs and outputs.

---

## 2. Fundamentals of Infrared Images

Explain infrared radiation, emissivity, and image formation.

### 2.1. Imaging System

Overview of lens, sensor, and processing pipeline.

#### Difficulties with the Lens

- Material limitations
- Optical aberrations
- Temperature sensitivity

#### Difficulties with Sensors

- Noise (NETD)
- Non-uniformity
- Dynamic range

#### Difficulties with Processors

- Real-time constraints
- Calibration
- Quantization effects

### 2.2. IR Image Synthesis Model

Describe synthesis models, assumptions, and pipelines.

### 2.3. Applications

#### Medical biochemical engineering

##### Pharmaceutical industry

##### Medical science

##### Cellular observations

##### Fluorescence microscopy

#### Vision tasks

##### Image conversion

##### Multispectral matching

##### Targets detection

##### Face recognition

#### Other engineering tasks

##### Automated vehicle

##### Remote sensing

##### Terrain models

##### Land surface

##### Food quality control

##### Agricultural management

##### Water resource management

##### Star formation

---

## 3. Related Methods

Summarize related methods and literature.

## Dataset
👉 [View Dataset Page](https://jyx51414.github.io/infrared-technology/dataset.html)


